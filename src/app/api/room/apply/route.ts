...
export async function POST(req: NextRequest) {
  ...
  const faith_description = (data.faith_description ?? '').trim()
  ...
  if (faith_description.length < 20)
    return NextResponse.json(
      { error: 'Please describe your Christian faith (at least 20 characters).' },
      { status: 400 },
    )
  if (!agreed_to_participate)
    return NextResponse.json({ error: 'You must agree to participate actively.' }, { status: 400 })

  const result = await submitApplication({ name, email, why_join, faith_description, agreed_to_participate })
